...
/*  insert admin data into admin_info table (if not exists)*/
insert ignore into admin_info(admin_name,admin_username,admin_password)
...
insert ignore into blood(blood_group)
...
insert ignore into contact_query (query_name,query_mail,query_number,query_message) values
...
CREATE TABLE if not exists query_stat(
 ...
 query_type VARCHAR(45) NOT NULL,
  ...
  insert ignore into query_stat(id,query_type)
  values('1',"Read"),
  ('2',"Pending");
